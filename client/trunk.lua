...
AddEventHandler('qb-trunk:client:KidnapTrunk', function()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
            ...
            QBCore.Functions.Notify('Kisiyi kacirmadin!', 'error')
        end
    end
end)
...
AddEventHandler('qb-trunk:client:KidnapGetIn', function(veh)
    ...
    if Config.TrunkClasses[vehClass].allowed then
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            -- if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                QBCore.Functions.Notify('Bagajdasin.', 'success', 4000)
                                TrunkCam(true)
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                QBCore.Functions.Notify('Bagaj kapalı mı?', 'error', 2500)
                            end
                        end
                    else
                        QBCore.Functions.Notify('Henuz kimse yok?', 'error', 2500)
                    end 
                else
                    QBCore.Functions.Notify('Zati bagajdasin', 'error', 2500)
                end 
            else
        ...
        QBCore.Functions.Notify('Bu aracin bagajına giremiyon knki..', 'error', 2500)
    end
end)
...
AddEventHandler('qb-trunk:client:GetIn', function(isKidnapped)
    ...
    if closestVehicle ~= 0 then
                ...
                if not disabledCarCheck(closestVehicle) then
                    if not inTrunk then
                        if not isBusy then
                            ...
                            QBCore.Functions.Notify('Kimse iceride mi?', 'error', 2500)
                        end 
                    else
                        QBCore.Functions.Notify('Sen zati bagajdasin', 'error', 2500)
                    end 
                else
                    QBCore.Functions.Notify('Bu aracin bagaja giremessin..', 'error', 2500)
                end
            end, plate)
        else
            QBCore.Functions.Notify('Bu aracin bagaja giremessin..', 'error', 2500)
        ...
        QBCore.Functions.Notify('Görünürde arac yok..', 'error', 2500)
    end
end)
        ...
        if inTrunk then
            if not isKidnapped then
                ...
                if DoesEntityExist(vehicle) then
                    DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.75, '[E] Bagajdan cik')
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            QBCore.Functions.Notify('Bagaj acik mi?', 'error', 2500)
                        end
                    end

                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, '[G] Bagajı kapat')
                        ...
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, '[G] Bagajı ac')
                        if IsControlJustPressed(0, Keys["G"]) then
                            if not IsVehicleSeatFree(vehicle, -1) then
